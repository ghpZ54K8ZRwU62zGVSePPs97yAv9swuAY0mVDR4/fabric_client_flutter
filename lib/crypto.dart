...
String encodePublicKeyToPem(RSAPublicKey publicKey) {
  ...
  var encodedPublicKeySeqBytes = publicKeySeq.encode();
  var publicKeySeqBitString =
      ASN1BitString(stringValues: Uint8List.fromList(encodedPublicKeySeqBytes));

  var topLevelSeq = ASN1Sequence();
  topLevelSeq.add(algorithmSeq);
  topLevelSeq.add(publicKeySeqBitString);
  var encodedTopLevelSeqBytes = topLevelSeq.encode();
  var dataBase64 = base64.encode(encodedTopLevelSeqBytes);
...
String encodePrivateKeyToPem(RSAPrivateKey privateKey) {
  ...
  var encodedPrivateKeySeqBytes = privateKeySeq.encode();
  var publicKeySeqOctetString =
      ASN1OctetString(octets: Uint8List.fromList(encodedPrivateKeySeqBytes));
  var topLevelSeq = ASN1Sequence();
...
ASN1Sequence makePublicKeyBlock(RSAPublicKey publicKey) {
  ...
  var encodedPublicKeySequence = publicKeySequence.encode();
  ASN1BitString blockPublicKey = ASN1BitString(stringValues: encodedPublicKeySequence);
...
String generateRsaCsrPem(Map<String, String> attributes,
  ...
  outer.add(ASN1BitString(stringValues: CryptoUtils.rsaPrivateKeyModulusToBytes(privateKey)));
  var encodedOuterBytes = outer.encode();
  List<String> chunks =
      StringUtils.chunk(base64.encode(encodedOuterBytes), 64);
